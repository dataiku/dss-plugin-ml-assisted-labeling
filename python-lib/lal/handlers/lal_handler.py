...
class LALHandler(object):
    ...
    def label(self, data, user):
        ...
        self.logger.error("raw_data : " + str(raw_data))
        self.logger.error("data_id : " + str(data_id))
        serialized_label = self.classifier.format_for_saving(data.get('label')) if data.get('label') else None
